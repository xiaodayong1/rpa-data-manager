<template>
  <div class="app-container">
    ...
    <el-dialog :title="upload.title" :visible.sync="upload.open" width="700px" append-to-body>
      <el-form ref="uploadForm" :model="uploadForm" :rules="uploadRules" label-width="150px">
        <el-form-item label="解析类型" prop="handleType">
          <el-input v-model="uploadForm.handleType" placeholder="请输入解析类型" />
        </el-form-item>
        <span>农行:ABC,农发行:ADBC,交行:BCM,北京银行:BOB,中国银行:BOC,建行有表头:CCB1,建行无表头:CCB2,中信:CITIC</span>
        <span>招商:CMB,民生:CMBC,工商:ICBC,乐山商业:LSSY,新疆农信:XJNX,烟台银行:YANTAIBANK,宜宾商业:YBSY</span>
        <el-form-item label="账号" prop="tradeAccountNum">
          <el-input v-model="uploadForm.tradeAccountNum" placeholder="请输入账号" />
        </el-form-item>
        <el-form-item label="开户行" prop="tradeBankName">
          <el-input v-model="uploadForm.tradeBankName" placeholder="请输入开户行" />
        </el-form-item>
        <el-form-item label="户名" prop="tradeAccountName">
          <el-input v-model="uploadForm.tradeAccountName" placeholder="请输入户名" />
        </el-form-item>
...
<script>
import { listVerify, getVerify, delVerify, addVerify, updateVerify,listAccount,listBank,insertTransaction,updateTransaction,getVerifyByid,setBaseLine,checkTask,checkTransaction } from "@/api/system/verify";
...
export default {
  ...
  methods: {
    ...
    //   this.$modal.confirm('是否确认删除明细详情编号为"' + ids + '"的数据项？').then(function() {
